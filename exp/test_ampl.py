...
cluster_ips   = params['cluster_ips'][:3]
...
def main(cmd: CMDManager, tp: LogParser):
    ...
    for method in methods:
        project_dir = f"{home_dir}/{'SMART' if method == 'SMART' else 'Sherman'}"
        work_dir = f"{project_dir}/build"
        env_cmd = f"cd {work_dir}"

        for cache_size in cache_sizes:
            # change config
            is_BTree = method != 'SMART'
            sed_cmd = generate_sed_cmd('./include/Common.h', is_BTree, 8 if key_type == 'randint' else 32, value_size, cache_size, MN_num, span_size)
            ...
            YCSB_TEST = f"{env_cmd} && ./ycsb_test {CN_num} {client_num_per_CN} {coro_num} {key_type} {workload_name}"
