...
using K = u64;
using V = u64;
using leaf_t = Leaf<64, K, V>;  // lxc: span_size
...
using rolex_t = Rolex<K, V, leaf_t, leaf_alloc_t, remote_memory_t, 32>;  // lxc: epsilon 4 表示kv位置（而不是leaf位置）的前后误差
using learned_cache_t = LearnedCache<K, V, leaf_t, leaf_alloc_t, 32>;  // lxc: epsilon 4
