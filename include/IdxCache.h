...
struct IdxCacheEntry {
  ...
  mutable uint16_t footprint;
  mutable int32_t cache_entry_freq;

  IdxCacheEntry(const GlobalAddress& leaf_addr, int kv_idx, const Key& k) : leaf_addr(leaf_addr), kv_idx(kv_idx), footprint(key2fp(k)), cache_entry_freq(1) {}
...
inline bool IdxCache::add_to_cache(const GlobalAddress& leaf_addr, int kv_idx, const Key& k) {
  ...
  for (const auto& e : bucket) if (e && e->leaf_addr == leaf_addr && e->kv_idx == kv_idx) {
    if (e->footprint != key2fp(k)) {
      __sync_bool_compare_and_swap(&(e->footprint), e->footprint, key2fp(k));
